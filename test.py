from instruments import Eload, Equity, PSU
...
class Test:
    ...
    def _fm(self):
        if self._fm_state == 'START':
            ...
            if not -10 <= self._current_temperature <= 85:
                raise ValueError('Temperature range error')
            ...
            self.eload.write(f'CURR {self._initial_electric_current}')
            sleep(1)
            ...
            if self._output_power >= self._voltage_X:
                self._fm_state = 'STEP_COMPLETE'
            else:
                self._fm_state = 'CONFIG_ELOAD'

        elif self._fm_state == 'STEP_COMPLETE':

            if self._current_temperature == -10:
                self._fm_state = 'END'
            else:
